...
*As a new learner, I want to see a simple 2D shape (a rectangle) on the screen, so that I can understand the absolute basics of rendering graphics in a web browser.*

### 2. The Design (The Plan)

This flowchart outlines the steps required to render a 2D shape. The design separates the structural HTML from the behavioral JavaScript, which will be in two separate files.
...
graph TD
    ...
    subgraph "JavaScript File: script.js"
        ...
        D[Set the 'fillStyle' property of the context to a color];
